...
void forward_kinematics_swing_foot(double q_joints[],double P_FTinH[], double Or_FTinH[], double Trans [][3], int side)
{
...
// Trigonometric Variables
    ...
    // From here added by Hamed Razavi (Dec 06, 2016)
    P_cartesian[1] = P_cartesian[1];  // y component of the CoM of the Pelvis
    P_cartesian[2] = P_cartesian[2];  // z component of the CoM of the Pelvis
    P_cartesian[4] = P_cartesian[4];  // Pitch Angle of the Pelvis
    P_cartesian[5] = P_cartesian[5];  // Yaw Angle of the Pelvis

    // P_cartesian [0] = x pelvis
    // P_cartesian [1] = y pelvis
    // P_cartesian [2] = z pelvis
    // P_cartesian [3] = Roll pelvis
    // P_cartesian [4] = pitch pelvis
    // P_cartesian [5] = Yaw pelvis
    ...
    for (int i =0; i < 3; i++)
        for (int j =0; j < 3; j++)
            cdPR(i,j) = cdPR1[i][j];
    A_FTtoH = cdPR * A_FTtoH; // in this one imu orientation is included if cdPR is multiplied
    P_FTinH_vec = cdPR * P_FTinH_vec; //
    for (int i =0; i < 3; i++)
        P_FTinH[i] = P_FTinH_vec(i);
    double thrFT, thpFT, thyFT, temp;
    if (((fabs(A_FTtoH(0,2))-1.) >= -1e-15)  )
    {
        thrFT = atan2(A_FTtoH(2,1),A_FTtoH(1,1));
        thyFT = 0;
        if ((A_FTtoH(0,2) > 0.)  )
    ...
    {
        thrFT = atan2(-A_FTtoH(1,2),A_FTtoH(2,2)); //roll
        thpFT = asin(A_FTtoH(0,2)); //pitch
        thyFT = atan2(-A_FTtoH(0,1),A_FTtoH(0,0));
    }
    Or_FTinH[0] = thrFT;
    Or_FTinH[1] = thpFT;
    Or_FTinH[2] = thyFT;
