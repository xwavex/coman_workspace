...
void forward_kinematics(double q_left[6], double q_right[6], double p_left_m[], double p_right_m[], double rotOrient[3][3], double t_yaw[3][3])
{
    ...
    // compute feet position based on kinematics chain

    for (int i=0; i<3; i++)
    {
        p_left_v(i) = t_Left_t(i,3);
        p_right_v(i) = t_Right_t(i,3);
        for (int j=0; j<3; j++)
        {
