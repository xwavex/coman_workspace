...
public:
    ...
    int getMCSNum(void) { return _mcs.size();}
    int getFTSNum(void) { return _fts.size();}
    ...
    Dsp_Board * get_dsp_board(int id ) {
        try { return _boards.at(id); }
        catch (std::out_of_range &e ) { return 0; }
    }

    McBoard * get_mc_board(int id ) {
        try { return _mcs.at(id); }
        catch (std::out_of_range &e ) { return 0; }
    }

    FtBoard * get_ft_board(int id ) {
        try { return _fts.at(id); }
        catch (std::out_of_range &e ) { return 0; }
    }
...
private:
    ...
    std::bitset<64>   bcMask, bcScanMask;
