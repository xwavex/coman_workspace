...
Boards_ctrl::Boards_ctrl(const char *config) {
    ...
    doc = YAML::LoadFile(config);
    const YAML::Node& board_ctrl = doc["board_ctrl"];
    
    std::string iface = board_ctrl["eth_iface"].as<std::string>();
    expected_num_boards = board_ctrl["boards_num"].as<int>();
...
Boards_ctrl::~Boards_ctrl() {
    ...
    pthread_mutex_destroy(&udp_sock_mutex);
...
void Boards_ctrl::stop_rx_udp(){
    ...
    std::cout << "Joined rx_udp thread" << std::endl;
...
void * Boards_ctrl::rx_udp(void *_) {
    ...
    int                 size;
    uint8_t             buff[RX_UDP_BUFSIZE], computedChecksum;
    ...
    for (;;) {
        ...
        size = recv(kls->udp_sock, &buff, sizeof(buff), 0);
        ...
        // verify checksum
        computedChecksum = 0;
        for (int i = 0; i < size-1; i++) computedChecksum -= buff[i];
        if ( computedChecksum != buff[size-1] ) {
            DPRINTF("Checksum mismatch 0x%02X 0x%02X\n", computedChecksum , buff[size-1] );
...
int Boards_ctrl::sendUdpPkt(UDPCommPacket &pkt) {

    uint8_t buffer[MAX_PAYLOAD_SIZE];
    int buff_size = pkt.getContent(buffer);
    pthread_mutex_lock(&udp_sock_mutex);
    int nbytes_sent = sendto(udp_sock, buffer, buff_size, 0, (sockaddr *)&dest_addr, sizeof(dest_addr)); 
    pthread_mutex_unlock(&udp_sock_mutex);

    assert(buff_size == nbytes_sent);
...
int Boards_ctrl::start_stop_control(uint8_t start_stop, uint8_t ctrl_type) {
    ...
    // value 0x0 motor state remains unchanged 
...
int Boards_ctrl::start_stop_set_control(std::vector<int> boards_set, uint8_t start_stop, uint8_t ctrl_type) {
    ...
    for (auto it = boards_set.begin(); it != boards_set.end(); it++) {
        cmd[(uint8_t)*it-1] = start_stop_value;
    }
