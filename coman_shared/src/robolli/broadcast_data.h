 ...
 * - policy value 0x093F 0b100100111111
...
typedef struct {
	...
	int	 Target_pos;
	unsigned char _chk;
	const unsigned short get_policy(void) { return 0x093F; }
	const unsigned short get_extra_policy(void) { return 0x0001; }
	const unsigned char faults(void) { return Faults_0; }
	void fprint(FILE *fp) {
		fprintf(fp, "%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n", Position,Velocity,Torque,PID_out,PID_err,Current,Faults_0,Faults_1,Link_pos,Target_pos);
	}
	void sprint(char *buff, size_t size) {
		snprintf(buff, size, "%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n", Position,Velocity,Torque,PID_out,PID_err,Current,Faults_0,Faults_1,Link_pos,Target_pos);
	}
