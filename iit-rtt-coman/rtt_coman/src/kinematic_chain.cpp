                                      ...
                                      const bool pure_torque)
{
    ...
    if(setPID(ID, 0, 0, 0) && setPIDTorque(ID, 0, 0, 0))
    {
        ...
        int tries = 0;
        while(mc_board->getItem(GET_MOTOR_CONFIG2, NULL, 0, REPLY_MOTOR_CONFIG2, &(mc_board->_motor_config2), sizeof(mc_board->_motor_config2)) != 0)
        {
            tries++;
            if(tries == 10)
                return false;
        ...
        tries = 0;
        while(mc_board->getItem(GET_MOTOR_CONFIG, NULL, 0, REPLY_MOTOR_CONFIG, &(mc_board->_motor_config), sizeof(mc_board->_motor_config)) != 0)
        {
            tries++;
            if(tries == 10)
                return false;
