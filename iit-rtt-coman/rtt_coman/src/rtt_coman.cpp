    ...
    is_controlled(false)
{
    ...
    this->addOperation("setPID", &rtt_coman::setPID, this, RTT::ClientThread);
...
//                this, RTT::ClientThread);
    ...
    // imu_data_.setZero(10);
    // imu_port_out.setName("imu_port_out");
    // imu_port_out.setDataSample(imu_data_);
    

// imu_out_ang_rate, imu_out_acc
    // imu_out_ang_rate_data, imu_out_acc_data;
    // imu_out_ang_rate_data.setZero(3);
    // imu_out_ang_rate.setName("imu_ang_rate");
    // imu_out_ang_rate.setDataSample(imu_out_ang_rate_data);
    // addPort(imu_out_ang_rate);

    // imu_out_acc_data.setZero(3);
    // imu_out_acc.setName("imu_port_out_acc");
    // imu_out_acc.setDataSample(imu_out_acc_data);
    // addPort(imu_out_acc);
}
...
bool rtt_coman::configureHook() {
    if(_yaml_is_loaded && _models_loaded && !is_configured)
    {
        ...
        // std::cout << "Model has " << ft_srdf.size() << " Force/Torque Sensors" << std::endl;
        // std::cout << "Robot has " << _boards->getFTSNum()<< " Force/Torque Sensors" << std::endl;
    ...
    imu_port_out.setName("imu_port");
    imu_port_out.setDataSample(imu_data_);
    ports()->addPort(imu_port_out);
...
void rtt_coman::updateHook(){
    double tic = get_simulationTime();

    float               euler[3], acc[3], angRate[3]; // , quat[4];
    data_3DM_GX3_t      raw_data;
    test_imu->get_raw_data_sync(raw_data);
    test_imu->get_Euler(euler, 0);
    ...
    // test_imu->get_Acc_Ang(acc, angRate, 0);

    // imu_data_(4) = angRate[0];
    // imu_data_(5) = angRate[1];
    // imu_data_(6) = angRate[2];

    // imu_data_(7) = acc[0];
    // imu_data_(8) = acc[1];
    // imu_data_(9) = acc[2];

    imu_port_out.write(imu_data_);
    ...
    tmp = getForceTorqueSensorsFrames();
...
bool rtt_coman::startHook()
{
    ...
    test_imu = new nrt_imu_3DM_GX3_25("/dev/ttyACM0", 115200, CMD_EULER);

    if(success == 0){
        // test_imu = new nrt_imu_3DM_GX3_25("/dev/ttyACM0", 115200, CMD_QUATERNION);
        is_controlled = true;
        return true;}
